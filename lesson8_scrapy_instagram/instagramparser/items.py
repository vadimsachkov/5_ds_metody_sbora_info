...
class InstagramParserItem(scrapy.Item):
    ...
    subscribers = scrapy.Field() # список подписчиков персоны, нужен чтобы потом пробежаться по пользователям в базе и убрать у них из подписки персону , если они уже отписались позднее, т.е. их нет в этом списке
